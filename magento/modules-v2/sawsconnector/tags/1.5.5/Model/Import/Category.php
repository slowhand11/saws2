...
use saws\sawsconnector\Model\Enterprise\VersionFeaturesFactory;
use saws\sawsconnector\Model\Import\Proxy\Category\ResourceModelFactory as CategoryResourceModelFactory;
use saws\sawsconnector\ResourceModel\Import\Category\StorageFactory;
use saws\sawsconnector\Model\Enterprise\CategoryImportVersion;
...
class Category extends \Magento\ImportExport\Model\Import\AbstractEntity {
  ...
  /** @var bool|string */
  protected $symbolEmptyFields = '###EMPTY###';
  ...
  /**
   * @var \saws\sawsconnector\ResourceModel\Import\Category\StorageFactory
   */
  ...
  StringUtils $string, ScopeConfigInterface $scopeConfig, ImportFactory $importFactory, ImportExportHelper $resourceHelper, ResourceConnection $resource, ProcessingErrorAggregatorInterface $errorAggregator, StoreManagerInterface $storeManager, StorageFactory $storageFactory, CategoryModel $defaultCategory, CategoryAttributeCollection $attributeCollection, CategoryCollection $categoryCollection, EavConfig $eavConfig, ManagerInterface $eventManager, UploaderFactory $imageUploaderFactory, Filesystem $filesystem, VersionFeaturesFactory $versionFeatures, ObjectRelationProcessor $objectRelationProcessor, TransactionManagerInterface $transactionManager, CategoryResourceModelFactory $resourceFactory, array $data = []
  ) {
    parent::__construct(
            $string, $scopeConfig, $importFactory, $resourceHelper, $resource, $errorAggregator, $data
    );
  ...
  protected function initCategories() {
    ...
    foreach ($collection as $category) {
      ...
      if ($pathSize > 1) {
        ...
        $this->categoriesWithRoots[$structure[1]][$category->getId()] = $this->categoriesWithRoots[$rootCategoryName][$index];
      }
    }
  ...
  public function getCategoriesWithRoots($bForceReload = false) {
    if ($bForceReload) {
      $this->getCollection()->clear()->load();
      $this->initCategories();
    }
   ...
   * @throws \Exception
   */
  protected function reindexUpdatedCategories() {
      ...
      if ($idToDelete) {
        ...
        try {
          $this->objectRelationProcessor->delete(
                  $this->transactionManager, $this->_connection, $categoryEntityTable, $this->_connection->quoteInto('entity_id IN (?)', $idToDelete), ['entity_id' => $idToDelete]
          );
  ...
  public function validateRow(array $rowData, $rowNum) {
    ...
    if (isset($rowData[self::COL_ROOT]) && isset($rowData[self::COL_CATEGORY]) && isset($this->newCategory[$rowData[self::COL_ROOT]][$rowData[self::COL_CATEGORY]])
    ) {
    ...
    if (Import::BEHAVIOR_DELETE == $this->getBehavior()) {
      if (self::SCOPE_DEFAULT == $rowScope && !isset($this->categoriesWithRoots[$rowData[self::COL_ROOT]][$rowData[self::COL_CATEGORY]])
      ) {
        $this->addRowError(self::ERROR_CATEGORY_NOT_FOUND_FOR_DELETE, $rowNum, null, null, ProcessingError::ERROR_LEVEL_NOTICE);
        return false;
      }
      return true;
    }
  ...
  protected function getUrlKey($rowData) {
    ...
    if (!empty($rowData[CategoryModel::KEY_NAME]) && $rowData[CategoryModel::KEY_NAME] != '###EMPTY###') {
      $u = \Magento\Framework\App\ObjectManager::getInstance()->create('\Magento\Framework\Filter\FilterManager');
      return $u->translitUrl($rowData[CategoryModel::KEY_NAME]);
      //return $this->productUrl->formatUrlKey($rowData[self::COL_CATEGORY]);
    }

    if (!empty($rowData[self::COL_CATEGORY]) && $rowData[CategoryModel::COL_CATEGORY] != '###EMPTY###') {
      $u = \Magento\Framework\App\ObjectManager::getInstance()->create('\Magento\Framework\Filter\FilterManager');
      return $u->translitUrl($rowData[self::COL_CATEGORY]);
      //return $this->productUrl->formatUrlKey($rowData[self::COL_CATEGORY]);
    }
      ...
      foreach ($bunch as $rowNum => $rowData) {
        ...
        foreach (array_intersect_key($rowData, $this->attributes) as $attrCode => $attrValue) {
          if (!$this->attributes[$attrCode]['is_static']) {
            ...
            if ('display_mode' == $attrCode) {
              $attrValue = str_replace(array('products only', 'static block only', 'static block and products'), array('PRODUCTS', 'PAGE', 'PRODUCTS_AND_PAGE'), strtolower($attrValue));
            }


            foreach ($storeIds as $storeId) {
              if ('multiselect' == $attribute->getFrontendInput() && $attrValue !== null) {
  ...
  protected function uploadMediaFiles($fileName) {
    try {
      $strDestImagePath = $this->getUploader()->getDestDir();
      $strSrcImagePath = $this->getUploader()->getTmpDir();

      if (file_exists($strDestImagePath . '/' . basename($fileName)) && md5_file($strDestImagePath . '/' . basename($fileName)) == md5_file($strSrcImagePath . '/' . $fileName)) {
  ...
  protected function saveCategoryEntity(array $entityRowsIn, array $entityRowsUp) {
    ...
    if ($entityRowsUp) {
      $this->_connection->insertOnDuplicate(
              $this->entityTable, $entityRowsUp, [CategoryModel::KEY_PARENT_ID, CategoryModel::KEY_PATH, CategoryModel::KEY_POSITION, CategoryModel::KEY_LEVEL, 'children_count']
      );
    }
  ...
  function prepareCategoriesForUpdate($arrUpdateData) {
    $arrResult = array();
    foreach ($arrUpdateData as $arrData) {

      if ($arrData['entity_id']) {
        $objCat = \Magento\Framework\App\ObjectManager::getInstance()->create('\Magento\Catalog\Model\Category')->load($arrData['entity_id']);
        $objCat->setStoreId(0);

        if ($objCat->getId()) {
          if ($objCat->getName() != $this->getCategoryName($arrData)) {
            $objCat->setName($this->getCategoryName($arrData));
            $objCat->save();
            $this->getCategoriesWithRoots(true);
          }

          $arrParent = $this->getParentCategory($arrData);
          try {

            if ($arrParent) {
              if ($arrParent['entity_id'] != $objCat->getParentId()) { // try to move
                $objCat->move($arrParent['entity_id'], 0);
                $objCat->save();
                $this->getCategoriesWithRoots(true);
              }
            }
